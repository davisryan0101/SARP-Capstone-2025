...
def runCoolingAnalysis(chamberLength, chamberRadius, lengthToThroat, throatRadius, throatRadiusOfCurvature, engineLength, nozzleExitRadius, innerJacketThermalConductivity, innerJacketWallThickness, gasSpecificHeatRatio, gasPrandtlNumber, gasViscosity, gasHeatCapacity, gasTemperature, chamberPressure, cstar, gasMolecularWeight, fuel, coolantWaterMassContent, totalCoolantFlowRate, numberCoolingChannels, initialCoolantPressure, initialCoolantTemperature, channelWidth, channelDepth, channelAbsoluteRoughness, innerJacketElasticModulus, innerJacketThermalExpansion, innerJacketPoissonRatio):
    ...
    def getCoolantHeatTransferCoefficient(position, velocity, bulkTemperature, wallTemperature, pressure):
        ...
        return nusseltNumber * coolantBulkThermalConductivity / getChannelHydraulicDiameter(position)
    ...
    def getCoolantSegmentProperties(position, nextPosition, upstreamCoolantTemperature, upstreamCoolantPressure):
        ...
        return [gasSideWallTemp, coolantSideWallTemp, coolantTemperature, heatFlux, coolantPressure, coolantVaporPressure, maxJacketStress]
    ...
    def minimizeHeatFluxDifference(position, coolantVelocity, upstreamCoolantTemperature, upstreamCoolantPressure):
        gasSideWallTemp = scipy.optimize.minimize_scalar(heatFluxDifference, bounds=(initialCoolantTemperature, getRecoveryTemperature(position)), method='bounded', tol=0.1, args=(position, coolantVelocity, upstreamCoolantTemperature, upstreamCoolantPressure))['x'] # could experiment with tolerance here
    ...
    return [zpoints, engineRadius, machNumber, recoveryTemperature, gasStaticPressure, gasSideWallTemp, coolantSideWallTemp, coolantTemperature, heatFlux, coolantPressure, maxJacketStress]
...
chamberRadius = 0.02058 # [m]
lengthToThroat = 0.08349 # [m]
throatRadius = 0.00376 # [m]
throatRadiusOfCurvature = 0.01 # [m] what is a good value for this?
engineLength = 0.2825 # [m]
nozzleExitRadius = 0.05708 # [m]

# Inner Jacket
innerJacketThermalConductivity = 40 # [W/m/K]
innerJacketWallThickness = 0.0005 # [m]
...
gasSpecificHeatRatio = 1.22 # update with CEA values!
gasPrandtlNumber = 0.47 # update with CEA values!
gasViscosity = 0.0000793 # [Pa*s]
gasHeatCapacity = 2592 # [J/kg/K] from CEA
gasTemperature = 2432 # [K] from CEA
chamberPressure = 200 * 6895 # [Pa]
cstar = 1602.8 * 0.94 # [m/s]
...
totalCoolantFlowRate = 0.0561 # [kg/s]
numberCoolingChannels = 25
initialCoolantPressure = 300 / pascalToPsi # [Pa]
initialCoolantTemperature = 293 # [K] could vary depending on orbit thermals, but ethanol pretty constant properties for wide temp & pres ranges
...
channelDepth = 0.002 # [m] what is possible with our machining capabilities? what is optimal from an analysis side of things? seems like high aspect ratio (depth>width) is best
